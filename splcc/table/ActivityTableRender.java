...
public class ActivityTableRender extends DefaultTableCellRenderer {
		...
		public Component getTableCellRendererComponent(JTable table, Object value,
			...
			table.getColumnModel().getColumn({{loop.index + 11}}).setMaxWidth(13);
			table.getColumnModel().getColumn({{loop.index + 11}}).setResizable(false);
			{% endfor %}
			
			{% if data.option.categories|length > 0 %}
			table.getColumnModel().getColumn({{11 + data.option.properties|length}}).setMaxWidth(13);
			table.getColumnModel().getColumn({{11 + data.option.properties|length}}).setResizable(false);
			{% endif %}
			//
