...
public class SubmissionTableRender extends DefaultTableCellRenderer {
	...
	public Component getTableCellRendererComponent(JTable table, Object value,
		...
		table.getColumnModel().getColumn({{loop.index + 2}}).setMaxWidth(13);
		table.getColumnModel().getColumn({{loop.index + 2}}).setResizable(false);
		{% endfor %}
		
		{% if data.option.categories|length > 0 %}
		table.getColumnModel().getColumn({{2 + data.option.properties|length}}).setMaxWidth(13);
		table.getColumnModel().getColumn({{2 + data.option.properties|length}}).setResizable(false);
		{% endif %}
		//
