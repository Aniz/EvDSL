...
public class ReviewUpdateScreenP extends JInternalFrame  {
	...
	public ReviewUpdateScreenP() {
		...
		try {
			lblLastId.setText(String.valueOf(EvCoMainScreenP.facade.getReviewLastId()));
	...
	private class UpdateButtonAction  implements ActionListener{ 
		...
		public void actionPerformed(ActionEvent e) {
			...
			try {
				review = EvCoMainScreenP.facade.searchReview(idReview);
			...
			try {
				...
				review = EvCoMainScreenP.facade.searchReview(review.getIdReview());
				textArea.setText("");
	...
	private void carregarComboBoxSubmission(){
		try {
			List<Submission> list = EvCoMainScreenP.facade.getSubmissionList();
