...
import evco.ev.table.ActivityUserTableModel;
import evco.ev.table.UserTableModel;
...
public class ActivityUserManagementScreenP extends JInternalFrame {
	...
	private class InsertButtonAction  implements ActionListener{ 
		...
		public void actionPerformed(ActionEvent e) {
			...
			try {
				...
				user =  new UserTableModel(EvCoMainScreenP.facade.getUserList()).get(rowIndex);
				int idActivity = EvCoMainScreenP.facade.getActivityIdByName(comboBoxActivity.getSelectedItem().toString());
				//Verificando se o numero de registration é maior que o numero de participantes atual
				activity = EvCoMainScreenP.facade.searchActivity(idActivity);
				if(activity.getNumberOfParticipants() < activity.getRegistrationLimit()){
					...
					//Inserindo na tabela
					EvCoMainScreenP.facade.insertActivityUser(activityUser);		
					//Atualizando a tabela
					ActivityUserTableModel model;
					model = new ActivityUserTableModel(EvCoMainScreenP.facade.getActivitiesUsersById(idActivity));
					...
					EvCoMainScreenP.facade.updateActivity(activity);
					atualizarNumeroParticipantes();
					// Atualizando valor no registration quando uma atividade é adicionada
					int idEvent = EvCoMainScreenP.getFacade().getEventbyActivity(idActivity);
					int idUser = user.getIdUser();
					int idRegistration = EvCoMainScreenP.getFacade().searchRegistration(idUser, idEvent);
					
					EvCoMainScreenP.getFacade().addValue(activity.getValue(),idRegistration);
				}else{
	...
	private class RemoveButtonAction  implements ActionListener{ 
		...
		public void actionPerformed(ActionEvent e) {
			...
			Activity activity = null;
			try {
				activityUser = new ActivityUserTableModel(EvCoMainScreenP.facade.getActivityUserList()).get(rowIndex);
				EvCoMainScreenP.facade.removeActivityUser(activityUser);
				//Atualizando a tabela
				int idActivity;
				idActivity = EvCoMainScreenP.facade.getActivityIdByName(comboBoxActivity.getSelectedItem().toString());
				ActivityUserTableModel model;
				model = new ActivityUserTableModel(EvCoMainScreenP.facade.getActivitiesUsersById(idActivity));
				tableUserActivity.setModel(model);	
				...
				EvCoMainScreenP.getFacade().removeValue(activity.getValue(),idRegistration);
			} catch (RepositoryException e1) {
				JOptionPane.showMessageDialog(getContentPane(),
						e1.toString(), "Erro",
						JOptionPane.INFORMATION_MESSAGE);
	...
	private class SelectComboActivityAction  implements ActionListener{ 
		...
		public void actionPerformed(ActionEvent e) {
			...
			try {
				...
				int i;
				i = EvCoMainScreenP.facade.getActivityIdByName(comboBoxActivity.getSelectedItem().toString());
				//Atualizando a tabela
				ActivityUserTableModel model;
				model = new ActivityUserTableModel(EvCoMainScreenP.facade.getActivitiesUsersById(i));
				tableUserActivity.setModel(model);
	...
	private class SelectComboEventAction  implements ActionListener{ 
		...
		public void actionPerformed(ActionEvent e) {
			...
			try {
				...
				activities = EvCoMainScreenP.facade.getActivitiesByEvent(EvCoMainScreenP.facade.getEventIdByName(comboBoxEvent.getSelectedItem().toString()));
	...
	private void populateTableUsers(){
		try {
			...
			model = new UserTableModel(EvCoMainScreenP.facade.getUserList());
	...
	private void atualizarNumeroParticipantes(){
		...
		try {
			...
			IdActivity = EvCoMainScreenP.facade.getActivityIdByName(comboBoxActivity.getSelectedItem().toString());
			Activity activity = EvCoMainScreenP.facade.searchActivity(IdActivity);
