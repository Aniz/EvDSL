...
import evco.ev.data.Activity.TypeActivity;
...
import evco.ev.table.ActivityTableModel;
...
public class ActivityManagementScreenP extends JInternalFrame {
			...
			if (nameEvent.equals("")|| nameActivity.equals("") || descriptionActivity.equals("")
				...
				if (numberOfParticipants > registrationLimit){
					...
					try {
						...
						activity.setIdEvent(EvCoMainScreenP.facade.getEventIdByName(nameEvent));
						...
						EvCoMainScreenP.facade.insertActivity(activity);
	...
	private class RemoveButtonAction  implements ActionListener{ 
		...
		public void actionPerformed(ActionEvent e) {
			...
			try {
				Activity activity = new ActivityTableModel(EvCoMainScreenP.facade.getActivities()).get(rowIndex);
				EvCoMainScreenP.facade.removeActivity(activity.getIdActivity());
			...
			try {
				...
				if (nameEvent.equals("")|| nameActivity.equals("") || descriptionActivity.equals("")
					...
					try {
						...
						model = new ActivityTableModel(EvCoMainScreenP.facade.getActivityList());
						table.setModel(model);
					} catch (ActivityNotFoundException e1) {
						JOptionPane
						.showMessageDialog(
	...
	private class SelectButtonAction  implements ActionListener{ 
		...
		public void actionPerformed(ActionEvent e) {
			...
			try {
				activity=  new ActivityTableModel(EvCoMainScreenP.facade.getActivities()).get(rowIndex);
			
				lblLastIdActivity.setText(String.valueOf(activity.getIdActivity()));
				comboBoxEvent.setSelectedItem(EvCoMainScreenP.facade.searchEvent(activity.getIdEvent()).getEventName());
	...
	private void carregarLastId(){
		try {
			lblLastIdActivity.setText(String.valueOf(EvCoMainScreenP.facade.getActivityLastId()));
		} catch (RepositoryException e) {
			JOptionPane.showMessageDialog(getContentPane(),
					e.toString(), "Erro",
					JOptionPane.INFORMATION_MESSAGE);
