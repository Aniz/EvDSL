...
public class ReviewInsertScreenP extends JInternalFrame  {
	...
	private class ValidateButtonAction  implements ActionListener{ 
		...
		public void actionPerformed(ActionEvent e) {
			try {
				...
				boolean validacao = EvCoMainScreenP.facade.isThereReviewer(Integer.valueOf(textFieldUserId.getText()));
				if(validacao == true){
					List<Assignment> assignments = EvCoMainScreenP.facade.getAssignmentList();
					for(Assignment a : assignments){
						if(textFieldUserId.getText().equals(a.getIdReviwerUser())){
							submission = EvCoMainScreenP.facade.searchSubmission(a.getIdReviewSubmission());
			...
			if (date.equals("") || description.equals("") || titleSubmission.equals("") || status.equals("")) {
				...
				return;
					...
					try {
						...
						review.setIdSubmission(EvCoMainScreenP.facade.getSubmissionIdByTitle(titleSubmission));
						int round = verificarRound(); 
						review.setRound(round + 1);
						review.setResult(resultado);
						
						EvCoMainScreenP.facade.updateReview(review);
	...
	public int verificarRound(){
		...
		try {
			int submissionid = EvCoMainScreenP.facade.getSubmissionIdByTitle(title);
			reviews = EvCoMainScreenP.facade.getReviewList();
		...
		try {
			Submission submission = EvCoMainScreenP.facade.searchSubmission(review.getIdSubmission());
//			Author author = pegarAuthorSubmission(submission);
			User user = EvCoMainScreenP.facade.searchUser(Integer.valueOf(textFieldUserId.getText()));
...
//		try {
//			submissionAuthorList = EvCoMainScreenP.facade.getSubmissionAuthorList();
//		} catch (RepositoryException e) {
//			JOptionPane.showMessageDialog(getContentPane(),
//					e.toString(), "Erro",
//					JOptionPane.INFORMATION_MESSAGE);
...
//			boolean bool;
//			try {
//				bool = EvCoMainScreenP.facade.isThereSubmissionAuthor(authorSubmission);
//				if(bool == true){
//					authorResult = EvCoMainScreenP.facade.searchAuthor(authorSubmission.getIdAuthor());
//					break;
//				}
//			} catch (RepositoryException e) {
//				JOptionPane.showMessageDialog(getContentPane(),
