...
public class RegistrationInsertScreenP extends JInternalFrame {
	...
	public RegistrationInsertScreenP() {
		...
		try {
			lblNewLabel.setText(String.valueOf(RiseEventsMainScreenP.facade.getRegistrationLastId()));
	...
	private class InsertButtonAction  implements ActionListener{ 
		...
		public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					int idEvent = RiseEventsMainScreenP.getFacade().getEventIdByName(comboBoxEvent.getSelectedItem().toString());
					
					//Atualizar o valor atual, com o valor do maintrack do evento.
					float totalValue = RiseEventsMainScreenP.getFacade().getEventMainTrackValue(idEvent);
					textFieldTotalValue.setText(String.valueOf(totalValue));
					
					
					registration.setIdEvent(idEvent);
					registration.setIdUser(RiseEventsMainScreenP.facade.getUserIdByName(nameUser));
					registration.setTotalValue(totalValue);

					RiseEventsMainScreenP.facade.insertRegistration(registration);
					
					//adicao na tabela de activityuser.
					ActivityUser activityUser = new ActivityUser();
					activityUser.setIdActivity(RiseEventsMainScreenP.getFacade().getActivityMainTrackId(idEvent));
					activityUser.setIdUser(registration.getIdUser());
					RiseEventsMainScreenP.getFacade().insertActivityUser(activityUser);
