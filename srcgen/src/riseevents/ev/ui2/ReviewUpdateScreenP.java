...
public class ReviewUpdateScreenP extends JInternalFrame  {
	...
	public ReviewUpdateScreenP() {
		...
		try {
			lblLastId.setText(String.valueOf(RiseEventsMainScreenP.facade.getReviewLastId()));
	...
	private class UpdateButtonAction  implements ActionListener{ 
		...
		public void actionPerformed(ActionEvent e) {
			...
			try {
				review = RiseEventsMainScreenP.facade.searchReview(idReview);
			...
			try {
				...
				review = RiseEventsMainScreenP.facade.searchReview(review.getIdReview());
				textArea.setText("");
	...
	private void carregarComboBoxSubmission(){
		try {
			List<Submission> list = RiseEventsMainScreenP.facade.getSubmissionList();
