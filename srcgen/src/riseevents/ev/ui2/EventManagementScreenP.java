...
import riseevents.ev.table.EventTableModel;
...
public class EventManagementScreenP extends JInternalFrame {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (eventName.equals("") || period.equals("") || place.equals("")
				...
				try {
					...
					//Atualizar JTable
					EventTableModel model = new EventTableModel(RiseEventsMainScreenP.facade.getEventList());

					RiseEventsMainScreenP.facade.insertEvent(event); //isso obriga que o programa seja executado a partir da tela main screen, caso ele seja iniciado da tela de login ficaria RISEEVENTLOGINSCREEN.facade....
	...
	private class RemoveButtonAction  implements ActionListener{ 
		...
		public void actionPerformed(ActionEvent e) {
			...
			try {
				Event event = new EventTableModel(RiseEventsMainScreenP.facade.getEventList()).get(rowIndex);
				RiseEventsMainScreenP.facade.removeEvent(event.getIdEvent());
		...
		public void actionPerformed(ActionEvent e) {
			...
			try {
				...
				if (eventName.equals("") || period.equals("") || place.equals("")
					...
					try {
						RiseEventsMainScreenP.facade.updateEvent(eventNew);
						EventTableModel model;
						model = new EventTableModel(RiseEventsMainScreenP.facade.getEventList());
						table.setModel(model);
					} catch (EventNotFoundException e1) {
						JOptionPane
						.showMessageDialog(
	...
	private class SelectButtonAction  implements ActionListener{ 
		...
		public void actionPerformed(ActionEvent e) {
			...
			try {
				eventOld=  new EventTableModel(RiseEventsMainScreenP.facade.getEventList()).get(rowIndex);
	...
	private void loadLastIndex(){
		try {
			lblLastEventId.setText(String.valueOf(RiseEventsMainScreenP.facade.getEventLastId()));
